	...
	while (!close) {
		...
		if (!fl.size()) {
			...
			if (selected - fl.dirCount() < screens.size() && screens[selected - fl.dirCount()] != "") {
				gmenu2x->s->box(480 - animation, gmenu2x->listRect.y, gmenu2x->skinConfInt["previewWidth"], gmenu2x->listRect.h, gmenu2x->skinConfColors[COLOR_TOP_BAR_BG]);

				// gmenu2x->sc[screens[selected - fl.dirCount()]]->softStretch(50, 50);
				gmenu2x->sc[screens[selected - fl.dirCount()]]->blit(gmenu2x->s, {480 - animation + padding, gmenu2x->listRect.y + padding, gmenu2x->skinConfInt["previewWidth"] - 2 * padding, gmenu2x->listRect.h - 2 * padding}, HAlignCenter | VAlignMiddle, 220);
